...
export async function create(req, res, next) {
  ...
  if (insertResultId) {
    ...
    const affectedRows = await stadiumRepository.insertStadiumConfig(
      config_data_arr
    );
    ...
    if (affectedRows > 0) res.json({ status: true, url: "/stadium" });
      ...
      res.json({
        status: false,
        error: "----- config insert affectedRows is 0 -----",
      });
  }
  // INSERT 쿼리 중 에러 발생
...
export async function editPage(req, res, next) {
  ...
  // 구장 매치 설정 조회
  const stadium_config_data = await stadiumRepository.getStadiumConfig(
    stadium_id
  );
  if (!stadium_config_data || stadium_config_data.length <= 0)
    data_object.config_data = null; // 등록시 필수 기입이라 null일리는 없긴함
...
export async function update(req, res, next) {
  ...
  if (updateResult) {
    const config_data_arr = getConfigParameterArray(
      formData.stadium_id,
      formData
    );
